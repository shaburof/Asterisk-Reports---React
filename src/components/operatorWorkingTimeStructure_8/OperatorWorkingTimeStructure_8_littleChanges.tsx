...
const OperatorWorkingTimeStructure_8: React.FC<typeOperatorWorkingTimeState> = (props) => {
    ...
    useEffect(() => {
        ...
        // console.log('devDataOperatorWorkingTimeStructure_8: ', devDataOperatorWorkingTimeStructure_8);
    ...
    const preparedData = (): any[] => {
        ...
        for (const operatorName in props.result) {
            ...
            for (const ext in exts) {
                (typeof temp[1] === 'undefined') ? temp[1] = exts[ext]['totalOperatorTime_41'] : temp[1] += exts[ext]['totalOperatorTime_41'];
                (typeof temp[2] === 'undefined') ? temp[2] = exts[ext]['$totalTimeSpentOnService_40'] : temp[2] += exts[ext]['$totalTimeSpentOnService_40'];
                (typeof temp[3] === 'undefined') ? temp[3] = exts[ext]['$operatorWorkload_14'] : temp[3] += exts[ext]['$operatorWorkload_14'];
                (typeof temp[4] === 'undefined') ? temp[4] = exts[ext]['$totalTimeSpentOnServicingIncomingCalls_28'] : temp[4] += exts[ext]['$totalTimeSpentOnServicingIncomingCalls_28'];
                (typeof temp[5] === 'undefined') ? temp[5] = exts[ext]['$totalTimeSpentOnExternalOutgoingCalls_29'] : temp[5] += exts[ext]['$totalTimeSpentOnExternalOutgoingCalls_29'];
                (typeof temp[6] === 'undefined') ? temp[6] = exts[ext]['$totalTimeSpentOnInternalOutgoingCalls_30'] : temp[6] += exts[ext]['$totalTimeSpentOnInternalOutgoingCalls_30'];
                (typeof temp[7] === 'undefined') ? temp[7] = exts[ext]['$totalPostCallTime_31'] : temp[7] += exts[ext]['$totalPostCallTime_31'];
                (typeof temp[8] === 'undefined') ? temp[8] = exts[ext]['onlineChatHours_32'] : temp[8] += exts[ext]['onlineChatHours_32'];
                (typeof temp[9] === 'undefined') ? temp[9] = exts[ext]['totalBreakTime_34'] : temp[9] += exts[ext]['totalBreakTime_34'];
                (typeof temp[10] === 'undefined') ? temp[10] = exts[ext]['$totalTimeWaitingForCall_35'] : temp[10] += exts[ext]['$totalTimeWaitingForCall_35'];
            }
        ...
        preparedDataValues = preparedDataValues.map(value => {
            value[1] = numberToStringTime(value[1]);
            value[2] = numberToStringTime(value[2]);
            value[3] = value[3] + ' %';
            value[4] = numberToStringTime(value[4]);
            value[5] = numberToStringTime(value[5]);
            value[6] = numberToStringTime(value[6]);
            value[7] = numberToStringTime(value[7]);
            value[8] = numberToStringTime(value[8]);
            value[9] = numberToStringTime(value[9]);
            value[10] = numberToStringTime(value[10]);
            return value;
        })
        // console.log('preparedDataValues: ', preparedDataValues);
